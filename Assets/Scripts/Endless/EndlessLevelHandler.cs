...
public class EndlessLevelHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            //Instantiate(sectionsPrefabs[prefabIndex]);
        ...
        for(int i=0; i<sections.Length; i++)
        {
            ...
            randomSection.transform.position = new Vector3(sectionsPool[i].transform.position.x, -50, i * sectionLength);
    ...
    void UpdateSectionPositions()
    {
        for (int i=0; i<sections.Length;i++)
        {
            if (sections[i].transform.position.z - playerCarTransform.position.z < -sectionLength)
            {
                ...
                sections[i].transform.position = new Vector3(LastSectionPosition.x, -50, LastSectionPosition.z + sectionLength*sections.Length);
